...
func (s *Service) Generate(ctx context.Context, req Request) (*Draft, error) {
	...
	text, _, err := s.llm.Complete(ctx, shared.CompletionRequest{Model: "gemini-2.0-flash", SystemPrompt: "Return valid JSON only.", Messages: []shared.Message{{Role: "user", Content: prompt}}, Temperature: 0.2, MaxTokens: 700})
	if err != nil {
		return nil, err
	}
	draft := &Draft{}
