...
func (s *Service) Discussion(ctx context.Context, req DiscussionRequest) (*DiscussionResponse, error) {
	text, _, err := s.llm.Complete(ctx, shared.CompletionRequest{Model: "gpt-4o-mini", SystemPrompt: "Summarize the discussion in 3-5 sentences.", Messages: append([]shared.Message{{Role: "user", Content: req.Question}}, req.Messages...), Temperature: 0.2, MaxTokens: 250})
	if err != nil {
		return nil, err
	}
