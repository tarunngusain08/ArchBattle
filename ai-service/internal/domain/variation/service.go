...
func (s *Service) Generate(ctx context.Context, req Request) (*Response, error) {
	...
	text, _, err := s.llm.Complete(ctx, shared.CompletionRequest{Model: "claude-3-5-sonnet-latest", SystemPrompt: "Produce one variation per line.", Messages: []shared.Message{{Role: "user", Content: "Create structural variations for: " + req.BasePrompt}}, Temperature: 0.4, MaxTokens: 600})
	if err != nil {
		return nil, err
	}
